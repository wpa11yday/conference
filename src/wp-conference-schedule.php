 ...
 *
 * @param array $args Array of query arguments to override defaults.
 */
function wpcsp_donors_list( $args = array() ) {
	$defaults = array(
...
function wpad_google_cal( $session_id ) {
	...
	$args = array(
		...
		'details' => urlencode( wp_unslash( trim( $excerpt ) ) ),
		'sf'      => 'true',
		'output'  => 'xml',
	);
...
function wpad_outlook_cal( $session_id ) {
	...
	$time    = get_post_meta( $session_id, '_wpcs_session_time', true );
	$time    = ( $time ) ? $time : strtotime( get_option( 'wpad_start_time' ) );
	$start   = gmdate( 'Y-m-d\THi00\Z', $time );
	$end     = gmdate( 'Y-m-d\THi00\Z', $time + 50 * 60 );
	$excerpt = get_the_excerpt( $session_id );
	...
	$args = array(
		...
		'body'    => urlencode( wp_unslash( trim( $excerpt ) ) ),
	);
